...
def counts(db: Session):
    ...
    for status, count in db.query(CoordinatorRecord.employment_status, func.count(CoordinatorRecord.id)).filter(CoordinatorRecord.is_deleted.is_(False)).group_by(CoordinatorRecord.employment_status).all(): result[status.value] = count
    return result
