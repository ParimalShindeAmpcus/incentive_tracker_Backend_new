...
def create_coordinator(payload: CoordinatorInput, db: DbSession, user: Annotated[User, Depends(require_roles("ADMIN", "ACCOUNTS"))]): return coordinator_service.create(db,payload)
@router.post("/bulk-upload", response_model=BulkUploadResponse)
async def bulk_upload(db: DbSession, user: Annotated[User, Depends(require_roles("ADMIN", "ACCOUNTS"))], file: UploadFile = File(...)):
    from app.services.common.deps import validate_upload_size

    validate_upload_size(file)
    return coordinator_service.bulk_upload(db, await file.read(), file.filename or "coordinators.csv")
@router.post("/bulk-mark-left", response_model=BulkMarkLeftResponse)
async def bulk_mark_left(db: DbSession, user: Annotated[User, Depends(require_roles("ADMIN", "ACCOUNTS"))], file: UploadFile = File(...)):
    from app.services.common.deps import validate_upload_size

    validate_upload_size(file)
    return coordinator_service.bulk_mark_left(db, await file.read(), file.filename or "left-coordinators.csv")
...
def update_coordinator(coordinator_id:int,payload:CoordinatorUpdate,db:DbSession,user: Annotated[User, Depends(require_roles("ADMIN", "ACCOUNTS"))]): return coordinator_service.update(db,coordinator_id,payload)
@router.post("/{coordinator_id}/employment-status", response_model=CoordinatorOut)
def update_status(coordinator_id:int,payload:CoordinatorStatusUpdate,db:DbSession,user: Annotated[User, Depends(require_roles("ADMIN", "ACCOUNTS"))]): return coordinator_service.update_status(db,coordinator_id,payload)
@router.delete("/{coordinator_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_coordinator(coordinator_id:int,db:DbSession,user: Annotated[User, Depends(require_roles("ADMIN", "ACCOUNTS"))]): coordinator_service.delete_left(db,coordinator_id)
