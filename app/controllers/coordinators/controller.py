...
def create_coordinator(payload: CoordinatorInput, db: DbSession, user: CurrentUser): return coordinator_service.create(db,payload,user=user)
@router.post("/bulk-upload", response_model=BulkUploadResponse)
async def bulk_upload(db: DbSession, user: CurrentUser, file: UploadFile = File(...)): return coordinator_service.bulk_upload(db, await file.read(), file.filename or "coordinators.csv", user=user)
@router.post("/bulk-mark-left", response_model=BulkMarkLeftResponse)
async def bulk_mark_left(db: DbSession, user: CurrentUser, file: UploadFile = File(...)): return coordinator_service.bulk_mark_left(db, await file.read(), file.filename or "left-coordinators.csv", user=user)
...
def update_coordinator(coordinator_id:int,payload:CoordinatorUpdate,db:DbSession,user:CurrentUser): return coordinator_service.update(db,coordinator_id,payload,user=user)
@router.post("/{coordinator_id}/employment-status", response_model=CoordinatorOut)
def update_status(coordinator_id:int,payload:CoordinatorStatusUpdate,db:DbSession,user:CurrentUser): return coordinator_service.update_status(db,coordinator_id,payload,user=user)
@router.delete("/{coordinator_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_coordinator(coordinator_id:int,db:DbSession,user:CurrentUser): coordinator_service.delete_left(db,coordinator_id,user=user)
