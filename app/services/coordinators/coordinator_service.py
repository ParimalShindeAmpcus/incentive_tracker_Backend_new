...
def update(db: Session, record_id: int, payload: CoordinatorUpdate):
    ...
    for field in ("organization", "role_title", "bank_name", "account_number", "ifsc_code"):
...
def bulk_upload(db, content: bytes, filename: str):
    ...
    for index,row in enumerate(_rows(content, filename), 2):
        try:
            payload=CoordinatorInput(full_name=row.get("Coordinator Name") or row.get("Full Name") or "", email=row.get("Email") or "", organization=row.get("Organization") or "", role_title=row.get("Role") or row.get("Role / Title") or "", employment_status=row.get("Employment Status") or "ACTIVE", bank_name=row.get("Bank Name") or None, account_number=row.get("Account Number") or None, ifsc_code=row.get("IFSC Code") or None)
