...
class TestDecisionMatrix:
    ...
    def test_multi_month_hours_stay_on_one_identity(self, matcher: ReconciliationMatcher):
        ...
        assert row["total_hours"] == 32

    def test_different_year_does_not_auto_match(self, matcher: ReconciliationMatcher):
        results = matcher.match(
            [_tpl(1, "John Smith", "Microsoft", month="2026-07")],
            [
                _group(
                    "John Smith",
                    "Microsoft",
                    month="2025-08",
                    hours=40,
                    monthly_hours={"2025-06": 40, "2025-07": 80, "2025-08": 32},
                )
            ],
            "2026-07",
        )
        row = next(
            ...
            for rows in results.values()
            for r in rows
            if r.get("template_candidate_name") == "John Smith"
        )
        assert row["match_status"] == "unmatched"
        flags = (row.get("match_explanation") or {}).get("identity_flags") or []
        assert "year_mismatch" in flags or "month_year_mismatch" in flags
        assert row.get("total_hours") == 0

    def test_same_year_other_months_is_matched_with_zero_hours(self, matcher: ReconciliationMatcher):
        """Person exists in the client file in the same year; June hours may be 0."""
        results = matcher.match(
            [_tpl(1, "Traci Davis", "CareFirst", month="2025-06")],
            [
                _group(
                    "Traci Davis",
                    "CareFirst:4185",
                    month="2025-08",
                    hours=118.5,
                    monthly_hours={"2025-07": 0, "2025-08": 118.5},
                )
            ],
            "2025-06",
        )
        row = _row(results, "Traci Davis")
        assert row["match_status"] == "matched"
        assert row["total_hours"] == 0
        monthly = row.get("monthly_hours") or {}
        assert monthly.get("2025-08") == 118.5

    def test_same_month_year_still_auto_matches(self, matcher: ReconciliationMatcher):
        results = matcher.match(
            [_tpl(1, "John Smith", "Microsoft", month="2025-06")],
            [
                _group(
                    ...
                    month="2025-06",
                    hours=40,
                    monthly_hours={"2025-06": 40, "2025-07": 80},
                )
            ],
            "2025-06",
        )
        assert _status_of(results, "John Smith") == "matched"
        assert _row(results, "John Smith")["total_hours"] == 40

    def test_multi_month_template_rows_are_not_homonyms(self, matcher: ReconciliationMatcher):
        templates = [
            _tpl(1, "John Smith", "Microsoft", month="2025-06", candidate_id="C1"),
            _tpl(2, "John Smith", "Microsoft", month="2025-07", candidate_id="C1"),
            _tpl(3, "John Smith", "Microsoft", month="2025-08", candidate_id="C1"),
        ]
        groups = [
            _group(
                "John Smith",
        ...
        rows = [
            r
            for status_rows in results.values()
            for r in status_rows
            if r.get("template_candidate_name") == "John Smith"
        ]
        assert len(rows) == 3
        assert {r["match_status"] for r in rows} == {"matched"}
        by_month = {r.get("messy_month"): r.get("total_hours") for r in rows}
        assert by_month["2025-06"] == 40
        assert by_month["2025-07"] == 80
        assert by_month["2025-08"] == 32


class TestResolveTargetMonth:
    def test_does_not_fall_back_to_unrelated_client_month(self):
        from app.services.vlookup.vlookup_service import _resolve_target_month

        assert (
            _resolve_target_month(
                explicit=None,
                template_months=["2026-07"],
                available_months=["2025-06", "2025-07", "2025-08"],
            )
            == "2026-07"
        )

    def test_prefers_overlapping_month(self):
        from app.services.vlookup.vlookup_service import _resolve_target_month

        assert (
            _resolve_target_month(
                explicit=None,
                template_months=["2025-06", "2025-07", "2025-08"],
                available_months=["2025-06", "2025-07", "2025-08"],
            )
            == "2025-08"
        )
